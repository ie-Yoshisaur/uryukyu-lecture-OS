...
int recursively_list_files_with_long_information (int i_dir, int argc,  char *argv[], int directory_depth, DIR *stack) {
    ...
    if (directory_depth >= 10) {
        stack = realloc(stack, sizeof(DIR) * directory_depth + 1);
    }
    stack[directory_depth] = *dir;
    directory_depth += 1;
    ...
    while ((dirent = readdir(dir)) != NULL) {
        ...
        file_info.is_dir = (bool)S_ISDIR(fileStat.st_mode);
    ...
    if (i_dir != 1 || directory_depth != 1) {
        printf("\n");
    }
    if (argc > ONE_ARG || directory_depth != 1) {
        printf("%s:\n", argv[i_dir]);
    }
    printf("total %d\n", directory_info.total_size);
    print_file_info_array(directory_info, file_info_array);
    for(int i_file=0; i_file<directory_info.num_of_files; i_file++) {
        bool is_file_name_empty = strcmp(file_info_array[i_file].file_name, "") == 0 || strcmp(file_info_array[i_file].file_name, " ") == 0;
        bool is_current_dir = strcmp(file_info_array[i_file].file_name, ".") == 0;
        bool is_parent_dir = strcmp(file_info_array[i_file].file_name, "..") == 0;
        bool does_file_name_contain_slash = strstr(file_info_array[i_file].file_name, "/");
        if (is_file_name_empty || is_current_dir || is_parent_dir || does_file_name_contain_slash) {
            continue;
        }
        if (file_info_array[i_file].is_dir) {
            int directory_index = 1;
            int length_of_path = sizeof(argv[i_dir]) + sizeof(file_info_array[i_file].file_name) + 1;
            char path_of_file[length_of_path];
            if (argc == ZERO_ARG) {
                sprintf(path_of_file, "%s/%s", ".", file_info_array[i_file].file_name);
            } else {
                sprintf(path_of_file, "%s/%s", argv[i_dir], file_info_array[i_file].file_name);
            }
            char **path_of_directory_array = malloc(sizeof(path_of_file)*2);
            path_of_directory_array[0] = "";
            path_of_directory_array[1] = (char*)path_of_file;
            recursively_list_files_with_long_information(directory_index, ONE_ARG, path_of_directory_array, directory_depth, stack);
            free(path_of_directory_array);
        }
    }
    // } print files with long information
...
int main (int argc, char *argv[]) {
    ...
    // the initial depth is defined as 10 in the assignment
    // check https://ie.u-ryukyu.ac.jp/~kono/lecture/os/ex/problem/192.html
    DIR *stack = malloc(sizeof(DIR) * 10);

    for (int i_dir=0; i_dir<argc; i_dir++) {
        if(recursively_list_files_with_long_information(i_dir, argc, argv, directory_depth, stack)==1)
            return 1;
    }
    free(stack);
    return 0;
